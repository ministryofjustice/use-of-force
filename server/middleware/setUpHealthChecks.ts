...
export default function setUpHealthChecks(applicationInfo: ApplicationInfo): Router {
  ...
  const apiConfig = Object.entries(config.apis).filter(([, options]) => typeof options === 'object' && 'url' in options)

  const middleware = monitoringMiddleware({
    applicationInfo,
    healthComponents: apiConfig.map(([name, options]) => endpointHealthComponent(logger, name, options as any)),
  })
